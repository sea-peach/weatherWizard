...
function displayCurrentWeather(currentWeather) {
  ...
  const html = `
    <div><h3>${currentDay}</h3> <h3>${month}</h3> <h3>${numericalDay}</h3>
...
function displayHourlyForecast(hourlyForecastByDay) {
  ...
  function displayDailyForecast(dailyForecast) {
    ...
    dailyForecast.time.forEach((time, index) => {
      ...
      html += `<h4>${dayOfWeek}, ${month} ${numericalDay}</h4> <div><p>${dailyForecast.temperature_2m_min[index]}°C</p> <p>${dailyForecast.apparent_temperature_max[index]}°C</p></div>`;
    });
    html += '</section>';
